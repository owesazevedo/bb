...
function installTargets(
  fakes: FakeTarget[],
  compositions: import("../../src/services/plugins/plugin-environment-provider-registry.js").PluginEnvironmentCompositionRecord[] = [],
): void {
  ...
  it.each([
    ...
    async (sourceState) => {
      await withTestHarness(async (harness) => {
              ...
              provision: (context) => {
                ...
                return {
                  ...
                  environment: {
                    ...
                    hostId: host.id,
                    path:
                      context.projectCheckout?.path ?? "/personal/workspace",
                    ownsPath: false,
                  },
                };
              },
        ...
        if (sourceState === "missing" || sourceState === "clone-failure") {
          ...
          expect(
            JSON.stringify(
              listEvents(harness.db, { threadId: checkoutThread.id }),
            ),
          ).toContain("Setting up project on machine");
          ...
          if (sourceState === "clone-failure") {
            await reportQueuedCommandError(harness, clone, {
              errorCode: "clone_failed",
              errorMessage: "Repository access denied",
            });
            await vi.waitFor(() =>
              expect(getThread(harness.db, checkoutThread.id)?.status).toBe(
                "error",
              ),
            );
            expect(
              getMachineLaunch(harness.db, checkoutThread.id),
            ).toMatchObject({ phase: "ready", hostId: host.id });
            ...
            ).toContain("Repository access denied");
            return;
          }
        ...
        expect(
          listQueuedCommands(harness, "provider.installation.run"),
        ).toEqual([]);
...
describe("a provider-produced environment over its life", () => {
  ...
  it.each(["existing", "new"] as const)(
    "reprovisions a destroyed environment using its %s machine selection",
    async (machineType) => {
      await withTestHarness(async (harness) => {
        const asks: TestEnvironmentProviderContext[] = [];
        const { host, project } = seedTargetFixture(
        ...
        if (machineType === "new") {
          ...
          setPluginMachineProviderBridge({
            listMachineProviders: () => [machineRecord],
            getMachineProvider: (id) =>
              id === "replacement-machine" ? machineRecord : undefined,
            invokeProvider: async (_pluginId, _label, run) => ({
              ok: true,
              value: await run(),
            }),
            ...
            selection: {
              machine:
                machineType === "new"
                  ? {
                      type: "new",
                      machineProviderId: "replacement-machine",
                      inputs: null,
                    }
                  : { type: "existing", hostId: host.id },
              inputs: { image: "img", cpus: 2 },
            },
          },
        });
        ...
        if (machineType === "new") {
          const replacementKey = `${thread.id}:replacement:${host.id}`;
          expect(machineKeys).toEqual([replacementKey]);
          expect(getMachineLaunch(harness.db, thread.id)).toMatchObject({
            phase: "ready",
            hostId: host.id,
            attempt: 1,
          });
          expect(getMachineLaunch(harness.db, replacementKey)).toMatchObject({
            phase: "ready",
            hostId: replacementHost.id,
            attempt: 1,
          });
